 ...
 *
 */
if ( ! class_exists( 'FLSCTO_CSF_Setup' ) ) {
  class FLSCTO_CSF_Setup {
    ...
    public function __construct() {
      ...
      // Init action
      do_action( 'flscto_csf_init' );

      // Setup textdomain
      self::textdomain();

      add_action( 'after_setup_theme', array( 'FLSCTO_CSF', 'setup' ) );
      add_action( 'init', array( 'FLSCTO_CSF', 'setup' ) );
      add_action( 'switch_theme', array( 'FLSCTO_CSF', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'FLSCTO_CSF', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'FLSCTO_CSF', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'FLSCTO_CSF', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'FLSCTO_CSF', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'FLSCTO_CSF_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FLSCTO_CSF_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FLSCTO_CSF_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FLSCTO_CSF_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FLSCTO_CSF_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FLSCTO_CSF_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FLSCTO_CSF_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FLSCTO_CSF_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FLSCTO_CSF_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FLSCTO_CSF_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FLSCTO_CSF_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        foreach ( self::$args['taxonomy_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FLSCTO_CSF_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'FLSCTO_CSF_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( FLSCTO_CSF_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'FLSCTO_CSF_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FLSCTO_CSF_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FLSCTO_CSF_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FLSCTO_CSF_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'FLSCTO_CSF_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              FLSCTO_CSF_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function includes() {
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'FLSCTO_CSF_Field_'. $field ) && class_exists( 'FLSCTO_CSF_Fields' ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! apply_filters( 'flscto_csf_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'FLSCTO_CSF_Field_' . $field['type'];
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field_type ) ) {
        ...
        $classname = 'FLSCTO_CSF_Field_'. $field_type;
...
FLSCTO_CSF_Setup::init( __FILE__, false );
...
if ( ! class_exists( 'FLSCTO_CSF' ) ) {
  class FLSCTO_CSF extends FLSCTO_CSF_Setup{}
}
