 ...
 *
 */
if ( ! class_exists( 'FLSCTO_CSF_Field_typography' ) ) {
  class FLSCTO_CSF_Field_typography extends FLSCTO_CSF_Fields {
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'csf-webfontloader' ) ) {
        ...
        FLSCTO_CSF::include_plugin_file( 'fields/typography/google-fonts.php' );
      ...
      if ( $is_google ) {
        ...
        if ( $font_weight || $font_style ) {
          ...
          if ( ! empty( $style ) ) {
            ...
            FLSCTO_CSF::$webfonts[$method][$font_family][$style] = $style;
          }
        } else {
          FLSCTO_CSF::$webfonts[$method][$font_family] = array();
        }
        ...
        if ( ! empty( $this->value['extra-styles'] ) ) {
          foreach ( $this->value['extra-styles'] as $extra_style ) {
            if ( ! empty( $extra_style ) ) {
              ...
              FLSCTO_CSF::$webfonts[$method][$font_family][$extra_style] = $extra_style;
            }
          }
        }
        ...
        if ( ! empty( $this->value['subset'] ) ) {
          ...
          foreach ( $this->value['subset'] as $subset ) {
            if( ! empty( $subset ) ) {
              FLSCTO_CSF::$subsets[$subset] = $subset;
            }
          }
        }
